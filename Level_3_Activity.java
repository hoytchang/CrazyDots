...
public class Level_3_Activity extends Activity { //ActionBarActivity {
	//declare the view
		private BallView3 ballView;
		...
		protected void onCreate(Bundle savedInstanceState) {
			...
			int hi_score = prefs.getInt("L3_hi_score_key", 0); //0 is the default value
			
			fullscreen();
			
			ballView = new BallView3(this);
			...
			if (action == MotionEvent.ACTION_DOWN ) { //|| action == MotionEvent.ACTION_MOVE){
				...
				if( Math.abs(x-red_x) < 40 && Math.abs(y-red_y) < 40 ) {
					...
					if (score > hi_score) {
						...
						editor.putInt("L3_hi_score_key", score);
						editor.commit();
