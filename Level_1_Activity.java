...
import com.TurboGames.crazydots.BallView;
...
public class Level_1_Activity extends Activity {  //ActionBarActivity {
	//declare the view
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		int hi_score = prefs.getInt("L1_hi_score_key", 0); //0 is the default value
		...
		if (action == MotionEvent.ACTION_DOWN ) { //|| action == MotionEvent.ACTION_MOVE){
			...
			if( Math.abs(x-red_x) < 40 && Math.abs(y-red_y) < 40 ) {
				...
				if (score > hi_score) {
					...
					editor.putInt("L1_hi_score_key", score);
					editor.commit();
